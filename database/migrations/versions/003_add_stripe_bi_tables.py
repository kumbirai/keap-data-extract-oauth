...
def upgrade() -> None:
    ...
    op.create_index("ix_stripe_products_stripe_account_id", "stripe_products", ["stripe_account_id"])
    op.create_index("ix_stripe_products_active", "stripe_products", ["active"])
    op.execute("CREATE INDEX ix_stripe_products_created_desc ON stripe_products (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_products_metadata_gin ON stripe_products USING GIN (metadata)")

    op.create_index("ix_stripe_prices_product_id", "stripe_prices", ["product_id"])
    op.create_index("ix_stripe_prices_stripe_account_id", "stripe_prices", ["stripe_account_id"])
    op.create_index("ix_stripe_prices_active", "stripe_prices", ["active"])
    op.execute("CREATE INDEX ix_stripe_prices_metadata_gin ON stripe_prices USING GIN (metadata)")

    op.create_index("ix_stripe_coupons_stripe_account_id", "stripe_coupons", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_coupons_created_desc ON stripe_coupons (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_coupons_metadata_gin ON stripe_coupons USING GIN (metadata)")

    op.create_index("ix_stripe_subscriptions_customer_id", "stripe_subscriptions", ["customer_id"])
    op.create_index("ix_stripe_subscriptions_status", "stripe_subscriptions", ["status"])
    op.create_index("ix_stripe_subscriptions_stripe_account_id", "stripe_subscriptions", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_subscriptions_created_desc ON stripe_subscriptions (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_subscriptions_metadata_gin ON stripe_subscriptions USING GIN (metadata)")

    op.create_index("ix_stripe_invoices_customer_id", "stripe_invoices", ["customer_id"])
    op.create_index("ix_stripe_invoices_subscription_id", "stripe_invoices", ["subscription_id"])
    op.create_index("ix_stripe_invoices_status", "stripe_invoices", ["status"])
    op.create_index("ix_stripe_invoices_stripe_account_id", "stripe_invoices", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_invoices_created_desc ON stripe_invoices (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_invoices_metadata_gin ON stripe_invoices USING GIN (metadata)")

    op.create_index("ix_stripe_payment_intents_customer_id", "stripe_payment_intents", ["customer_id"])
    op.create_index("ix_stripe_payment_intents_status", "stripe_payment_intents", ["status"])
    op.create_index("ix_stripe_payment_intents_latest_charge_id", "stripe_payment_intents", ["latest_charge_id"])
    op.create_index("ix_stripe_payment_intents_stripe_account_id", "stripe_payment_intents", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_payment_intents_created_desc ON stripe_payment_intents (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_payment_intents_metadata_gin ON stripe_payment_intents USING GIN (metadata)")

    op.create_index("ix_stripe_charges_customer_id", "stripe_charges", ["customer_id"])
    op.create_index("ix_stripe_charges_status", "stripe_charges", ["status"])
    op.create_index("ix_stripe_charges_stripe_account_id", "stripe_charges", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_charges_created_desc ON stripe_charges (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_charges_metadata_gin ON stripe_charges USING GIN (metadata)")

    op.create_index("ix_stripe_refunds_charge_id", "stripe_refunds", ["charge_id"])
    op.create_index("ix_stripe_refunds_status", "stripe_refunds", ["status"])
    op.create_index("ix_stripe_refunds_stripe_account_id", "stripe_refunds", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_refunds_created_desc ON stripe_refunds (created DESC NULLS LAST)")
    op.execute("CREATE INDEX ix_stripe_refunds_metadata_gin ON stripe_refunds USING GIN (metadata)")

    op.create_index("ix_stripe_balance_transactions_type", "stripe_balance_transactions", ["type"])
    op.create_index("ix_stripe_balance_transactions_available_on", "stripe_balance_transactions", ["available_on"])
    op.create_index("ix_stripe_balance_transactions_source_id", "stripe_balance_transactions", ["source_id"])
    op.create_index("ix_stripe_balance_transactions_stripe_account_id", "stripe_balance_transactions", ["stripe_account_id"])
    op.execute(
        "CREATE INDEX ix_stripe_balance_transactions_created_desc ON stripe_balance_transactions (created DESC NULLS LAST)"
    )

    op.create_index("ix_stripe_payouts_status", "stripe_payouts", ["status"])
    op.create_index("ix_stripe_payouts_stripe_account_id", "stripe_payouts", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_payouts_created_desc ON stripe_payouts (created DESC NULLS LAST)")

    op.create_index("ix_stripe_transfers_status", "stripe_transfers", ["status"])
    op.create_index("ix_stripe_transfers_stripe_account_id", "stripe_transfers", ["stripe_account_id"])
    op.execute("CREATE INDEX ix_stripe_transfers_created_desc ON stripe_transfers (created DESC NULLS LAST)")
